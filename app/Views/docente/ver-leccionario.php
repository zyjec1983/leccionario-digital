        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <p><strong>Fecha:</strong> <?= date('d/m/Y', strtotime($leccionario['fecha'] ?? date('Y-m-d'))) ?></p>
                        <p><strong>Hora:</strong> <?= substr($leccionario['hora_inicio'] ?? '', 0, 5) ?> - <?= substr($leccionario['hora_fin'] ?? '', 0, 5) ?></p>
                        <?php if (!empty($leccionario['aula'])): ?>
                        <p><strong>Aula:</strong> <?= htmlspecialchars($leccionario['aula']) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <p>
                            <strong>Firma:</strong>
                            <?php if (!empty($leccionario['firmado'])): ?>
                ...
                <div class="text-muted small mb-4">
                    ...
                    Registrado el: <?= date('d/m/Y H:i', strtotime($leccionario['fecha_registro'] ?? date('Y-m-d H:i:s'))) ?>
                </div>

                                <?php 
                $esPasado = strtotime($leccionario['fecha'] ?? date('Y-m-d')) < strtotime(date('Y-m-d'));
                $puedeEditar = in_array($leccionario['estado'] ?? '', ['pendiente', 'atrasado']);
                ?>
                
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <?php if ($puedeEditar): ?>
                    <a href="<?= route('docente/leccionarios/nuevo/' . ($leccionario['horario_id'] ?? 0) . '/' . ($leccionario['fecha'] ?? '')) ?>" class="btn btn-primary">
